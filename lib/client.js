	...
	factory: (require) => {
		...
		function statusTextOf(account, translate) {
			...
			switch (account.status) {
				...
				case "unsupported": return translate("balance.unsupportedEndpoint");
				default: return translate("balance.error");
			}
		}
		//#endregion
		...
		const zh = {
			...
			"balance.unsupportedEndpoint": "接口地址不受支持（需 HTTPS 公网地址）",
			"balance.error": "查询失败",
		...
		const en = {
			...
			"balance.unsupportedEndpoint": "Unsupported endpoint (HTTPS public URL required)",
			"balance.error": "Query failed",
				...
				children: [
					...
					react_jsx_runtime.jsxs("div", {
						...
						children: [
							...
							account.error && react_jsx_runtime.jsx("p", { className: "b_note", children: account.error }),
		...
		const zhSettings = {
			...
			show: "在悬浮窗中显示该供应商",
			proxyMode: "代理模式",
			proxyModeHint: "开启后走系统网络/代理（跳过 DNS 固定校验，兼容 Clash / Surge / Shadowrocket 等代理工具）"
		};
		...
		const enSettings = {
			...
			proxyModeHint: "Use the system network/proxy (skips strict DNS pinning; compatible with Clash / Surge / Shadowrocket)"
		};
		...
		function BalanceSettingsCard({ api, remote, t }) {
			...
			const [proxyValue, setProxyValue] = react.useState(false);
			const [proxyOriginal, setProxyOriginal] = react.useState(false);
			const mountedRef = react.useRef(true);
			...
			const load = react.useCallback(async () => {
				...
				try {
					...
					setProxyOriginal(payload?.proxy === true);
					setProxyValue(payload?.proxy === true);
					const next = {};
			...
			const save = async () => {
				...
				try {
					const ops = [];
					if (proxyValue !== proxyOriginal) {
						if (proxyValue) ops.push({ op: "set", path: ["proxy"], value: true });
						else ops.push({ op: "unset", path: ["proxy"] });
					}
			...
			if (status === "loading") {
				...
				body = [
					...
					h("div", { className: "b_set_provider" },
						h("label", { className: "b_set_check" },
							h("input", { type: "checkbox", checked: proxyValue, disabled: !writable || busy, onChange: (event) => { setProxyValue(event.target.checked); setNotice(""); } }),
							" " + t("proxyMode")),
						h("p", { className: "b_set_hint" }, t("proxyModeHint"))),
					rows,
