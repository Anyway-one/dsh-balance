...
export async function configuredProviders(ctx) {
	...
	const overrides = balance !== void 0 && balance !== null && typeof balance === "object"
...
function validateBalanceMutation(body) {
	...
	for (const operation of body.ops) {
		...
		if (!Array.isArray(operation.path) || operation.path.length !== 3
			|| operation.path[0] !== "providers" || operation.path[2] !== "baseURL"
			|| typeof operation.path[1] !== "string" || operation.path[1] === "") {
			throw new Error('path must be ["providers", <id>, "baseURL"]');
		}
		const id = operation.path[1];
		if (seen.has(id)) throw new Error(`provider "${id}" appears more than once`);
		seen.add(id);
		if (operation.op === "set") {
...
/** Provider rows for the config card: display facts + credential ref + base URL. */
...
async function handleState(ctx, req, res) {
	...
	try {
		...
		json(res, 200, { ok: true, providers: await stateProviders(ctx), revision, writable });
	} catch (error) {
