/**
 * dsh-balance — safe upstream fetch for balance queries.
 *
 * HTTPS only, DNS pre-resolution with private-network rejection, and
 * connection pinning to the address that passed policy (DNS-rebinding
 * defense), plus a response size cap and timeout. The policy mirrors the
 * approach of dsh-usage-stats (MIT) / dsh-usage (MIT), reimplemented here so
 * this plugin stays self-contained.
 *
 * @module dsh-balance/safe-fetch
 */
...
export function ipv4Private(parts) {
	...
	// NOTE: 198.18.0.0/15 (RFC 2544 benchmarking) is intentionally NOT treated as
	// private — Clash/Surge and friends use it as their fake-ip pool, and it is not
	// a real routable private network, so rejecting it only breaks proxy users while
	// adding no real protection (real LAN ranges above stay blocked).
...
export function isPrivateAddress(address) {
	...
	// IPv4-mapped IPv6 (::ffff:0:0/96, RFC 4291), including the hex form
	// ::ffff:xxxx:xxxx and Shadowrocket/Clash's ::ffff:0:xxxx:xxxx variant.
	const mapped = (parts[5] === 0xffff && parts.slice(0, 5).every((w) => w === 0))
		|| (parts[4] === 0xffff && parts[5] === 0 && parts.slice(0, 4).every((w) => w === 0));
	if (mapped) {
...
export async function safeFetch(rawUrl, init = {}, deps = {}) {
	...
	// Proxy mode: use the system's normal DNS + connection (no pre-resolution, no
	// pinning, no private-address rejection) so proxy tools / local gateways work.
	const allowProxy = deps.allowProxy === true;
	const target = allowProxy ? { url } : { url, ...await resolvePublicAddress(url, deps) };
	...
	return new Promise((resolve, reject) => {
		...
		if (!allowProxy) {
			options.servername = isIP(target.url.hostname.replace(/^\[|\]$/g, "")) === 0 ? target.url.hostname : void 0;
		...
		const request = transport.httpsRequest(target.url, options, (response) => {
			const chunks = [];
			let size = 0;
