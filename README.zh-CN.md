...
每个已配置供应商一行，**内嵌在 sidebar 底部**（会话列表与设置脚注之间，宽度自适应 sidebar）。状态圆点 + 彩色数值一眼看清每个账户的健康状况；sidebar 收起时自动折叠成一枚小胶囊。
...
在 **设置 → 插件 → 插件配置 → 余额 Balance** 中图形化配置每个供应商的 API Key（写入 DSH 凭据存储）、可选的接口地址（baseURL）覆盖，以及每个供应商的**显示/隐藏**开关——可隐藏任意不需要的自动发现供应商（例如没有公开余额接口的 pi-ai profile）。无需手改 YAML。
...
**万能开关——代理模式**：如果上面的办法都覆盖不了你的代理 / 网关 / 分流 DNS 场景，打开 **设置 → 插件 → 插件配置 → 余额 Balance**，勾选 **代理模式**。它会跳过严格的 DNS 固定 + 公网校验，改走系统网络连接，任何代理都能通。（代价：余额查询会关闭 DNS 重绑定防护，不用代理时建议关掉。）
...
npm test              # 45 个离线测试：余额 scheme、安全边界、服务端边界、settings/mutate、历史缓冲
```
